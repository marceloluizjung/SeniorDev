...
import br.com.senior.varejo.pedidos.Fielddto;
import br.com.senior.varejo.pedidos.FileFormat;
import br.com.senior.varejo.pedidos.Hookdto;
import br.com.senior.varejo.pedidos.Layoutdto;
...
public final class LayoutDTOConverter {
	...
	public static Layoutdto toDto(Layout layout) {
	    ...
	    dto.errorStrategy = br.com.senior.varejo.pedidos.ErrorStrategy.valueOf(layout.getErrorStrategy().name());
	    ...
	    for (Field field : layout.getFields()) {
	        ...
	        fdto.fieldType = br.com.senior.varejo.pedidos.FieldType.valueOf(field.getFieldType().name());
	        fdto.id = field.getId();
	    ...
	    if (layout.getHooks() != null) {
	        for (Hook hook : layout.getHooks()) {
	            ...
	            hdto.hookFunction = br.com.senior.varejo.pedidos.HookFunction.valueOf(hook.getHookFunction().name());
